...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        pos_emb = F.interpolate(self.positional_embedding.permute(1, 0).unsqueeze(0), size=[x.shape[1]],
                                mode='linear').reshape(-1, x.shape[1]).permute(1, 0)
...
class VisionTransformer_Local(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        #     x = blk(x)
        # # for blk in self.transformer.resblocks[-1:]:
        # #     x = x + self.custom_attn(blk.attn, blk.ln_1(x), csa=True)
        ...
        # for blk in self.transformer.resblocks[-1:]:
        #     if ignore_residual:
        #         output += self.custom_attn(blk.attn, blk.ln_1(x), csa=csa_flag)
        #         x = blk(x)
        #     else:
        #         x_out = x + self.custom_attn(blk.attn, blk.ln_1(x), csa=csa_flag)
...
class CLIP_Local(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            # self.visual_global = VisionTransformer(
            #     input_resolution=image_resolution,
            #     patch_size=vision_patch_size,
            #     width=vision_width,
            #     layers=vision_layers,
        ...
        # print(self.context_length) #77
        # print(transformer_width) #512
    ...
    #     return self.visual_global(image.type(self.dtype))
...
def build_model(state_dict: dict, local_type: bool):
